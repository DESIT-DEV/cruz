...
public function insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$descripcion,$total_venta,$idarticulo,$cantidad,$precio_venta,$descuento){
	require "../config/Conexion.php";

	$sql="INSERT INTO venta (idcliente,idusuario,tipo_comprobante,serie_comprobante,num_comprobante,fecha_hora,impuesto,total_venta,estado,Descripcion) VALUES ('$idcliente','$idusuario','$tipo_comprobante','$serie_comprobante','$num_comprobante','$fecha_hora','$impuesto','$total_venta','Aceptado','$descripcion')";
...
public function anular($idventa){
	...
	$detalle=mysqli_query($conexion,"SELECT `idarticulo`,`cantidad` FROM `detalle_venta` WHERE `idventa`='$idventa'");
...
/*	$sql="SELECT a.nombre AS articulo, a.codigo, d.cantidad, d.precio_venta,d.Descripcion, d.descuento, (d.cantidad*d.precio_venta*d.descuento) AS subtotal FROM detalle_venta d INNER JOIN articulo a ON d.idarticulo=a.idarticulo WHERE d.idventa='$idventa'";
         return ejecutarConsulta($sql);
*/
 $sql="SELECT a.nombre AS articulo, a.codigo, d.cantidad, d.precio_venta, d.descuento,v.Descripcion,v.total_venta, (d.cantidad*d.precio_venta*d.descuento) AS subtotal FROM detalle_venta d INNER JOIN articulo a ON d.idarticulo=a.idarticulo inner join venta v on d.idventa=v.idventa WHERE d.idventa='$idventa'";
         return ejecutarConsulta($sql);
}
