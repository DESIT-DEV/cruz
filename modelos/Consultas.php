...
public function total($fecha_inicio,$fecha_fin){
	$sql="SELECT DISTINCT d.`idarticulo` from venta v inner join detalle_venta d on d.idventa=v.idventa WHERE DATE(v.fecha_hora)>='$fecha_inicio' AND DATE(v.fecha_hora)<='$fecha_fin'";
	return ejecutarConsulta($sql);
}
